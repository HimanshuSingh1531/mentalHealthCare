...
# --- Configuration ---
OPENROUTER_API_KEY = "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"
...
def run_streamlit_chatbot():
    ...
    if user_input:
        ...
        st.session_state.messages.insert(1, extra_prompt)
        st.session_state.messages.append({"role": "user", "content": user_input})

        with st.chat_message("user"):
            st.markdown(user_input)

        st.markdown(f"🧠 *Detected Emotion:* `{sentiment.upper()}` {emoji} (Confidence: {confidence:.2f})")

        bot_reply, _, _ = generate_chatbot_reply(user_input, st.session_state.messages[1:])
